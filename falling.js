...
scientist_falling.initialize = function () {
	...
	function initializeIntro () {
		...
		//multiverse.eventlistener('touchstart', introObj.elem, startGame);
		//multiverse.eventlistener('click', introObj.elem, startGame);
	...
	function superCreator(arr, prefix, callback) {
		...
		for(b = 0; b < nameArray.length; b+= 1) {
			...
			obj = objectCreator(superArray[nameArray[b]], 'human');
			obj.isHuman = true;
    ...
    function updateBubbles() {
		...
		//newbub.x = scientist[1].body.GetPosition().x * scale;
		//newbub.y = scientist[1].body.GetPosition().x * scale - container.y;
	...
	function setUpTube () {
		...
		tubeWallFix2.shape.SetAsBox(1/scale, (canvasheight * 200) /scale);
	...
	function steer (evt) {
		...
		//rotations = Math.atan2((y - canvasheight * 0.5), (x - canvaswidth * 0.5));
		
		motor.x = (directionVector * 0.2);
